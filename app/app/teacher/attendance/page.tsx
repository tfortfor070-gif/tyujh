...
export default function TeacherAttendancePage() {
  const { attendance, loading } = useTeacherData();

  if (loading) {
    return <div><PageHeader title="Présences" /><LoadingState /></div>;
  }

  return (
    <div>
      <PageHeader title="Présences" description="Historique des présences enregistrées" />
      <Card className="p-4">
        {attendance.length === 0 ? (
